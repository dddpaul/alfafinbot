...
func TestNewPurchase(t *testing.T) {
	p, err := purchases.New("Покупка 527,11 ₽, Озон. Карта **1111. Баланс: 4506,85 ₽")
	assert.Nil(t, err)
	...
	p, err = purchases.New("Покупка ₽, Озон. Карта **1111. Баланс: 4506,85 ₽")
	assert.NotNil(t, err)

	p, err = purchases.New("Покупка ABC ₽, Озон. Карта **1111. Баланс: 4506,85 ₽")
	assert.NotNil(t, err)

	p, err = purchases.New("Деньги пришли! 20 000 ₽ на карту **1111. Баланс: 21 945,39 ₽")
	assert.NotNil(t, err)
}
